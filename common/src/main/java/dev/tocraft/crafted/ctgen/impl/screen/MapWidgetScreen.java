...
import dev.tocraft.crafted.ctgen.impl.screen.widget.MapWidgetBuilder;
...
public class MapWidgetScreen extends Screen {
    private static final float MAP_SCALE = 0.875f;
    @Nullable
    private final MapWidget mapWidget;

    public MapWidgetScreen(Minecraft minecraft) {
        ...
        MapWidgetBuilder builder = MapWidgetBuilder.createDefault();
        if (builder != null) {
            builder.setMinecraft(minecraft);
            MapWidget widget = builder.build();
            setSpecs(widget);
    ...
    private void setSpecs(MapWidget widget) {
        ...
        widget.updateFrame(frameX, frameY, frameWidth, frameHeight);
    }
